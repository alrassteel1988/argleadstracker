...
const PUBLIC_STATIC_FILES = new Map([
  ...
  ["/ai-sales-assistant.css", "ai-sales-assistant.css"],
  ["/bauhaus-global.css", "bauhaus-global.css"],
  ["/lead-detail-contrast.css", "lead-detail-contrast.css"],
  ["/lead-detail-readability.css", "lead-detail-readability.css"],
  ["/pipeline-live-leads.css", "pipeline-live-leads.css"],
  ["/salesman-dashboard-live-leads.css", "salesman-dashboard-live-leads.css"],
  ["/salesman-dashboard-summaries.css", "salesman-dashboard-summaries.css"],
  ["/salesmen-directory.css", "salesmen-directory.css"],
  ["/tasks-bauhaus-flat.css", "tasks-bauhaus-flat.css"],
  ["/tasks-contrast.css", "tasks-contrast.css"],
  ["/sw.js", "sw.js"],
  ...
  ["/icons/shortcut-focus.png", "icons/shortcut-focus.png"],
  ["/icons/shortcut-log.png", "icons/shortcut-log.png"]
]);

function publicStaticFileForPath(pathname) {
  if (pathname === "/") return PUBLIC_STATIC_FILES.get("/index.html");
  if (/^\/leads\/[^/]+\/?$/.test(pathname)) return PUBLIC_STATIC_FILES.get("/index.html");
  return PUBLIC_STATIC_FILES.get(pathname) || "";
}
...
function activitySortDesc(a, b) {
  return String(b?.at || b?.activity_date || b?.created_at || "")
    .localeCompare(String(a?.at || a?.activity_date || a?.created_at || ""));
}

function leadActivitiesNewestFirst(lead) {
  return ensureActivityIds(lead?.activities || []).sort(activitySortDesc);
}

function isReminderLikeActivity(activity) {
  return Boolean(
    activity?.reminder
    || activity?.due_date
    || activity?.due_time
    || activity?.reminder_type
    || String(activity?.type || "").toLowerCase() === "reminder"
  );
}

function isCallActivity(activity) {
  const type = String(activity?.type || "").toLowerCase();
  return type.includes("call") || type === "phone";
}

function isEmailActivity(activity) {
  return String(activity?.type || "").toLowerCase().includes("email");
}

function isMeetingActivity(activity) {
  const type = String(activity?.type || "").toLowerCase();
  return type.includes("meeting") || type.includes("visit") || type.includes("video");
}

function isStageActivity(activity) {
  return String(activity?.type || "").toLowerCase() === "stage";
}

function isNoteActivity(activity) {
  const type = String(activity?.type || "").toLowerCase();
  return type === "note" || (!type && String(activity?.text || activity?.note || "").trim());
}

function isQuoteActivity(activity) {
  const type = String(activity?.type || "").toLowerCase();
  return Boolean(activity?.quotation_ref) || type.includes("quotation") || type.includes("quote");
}

function normalizeReminderHistory(activity) {
  return {
    ...activity,
    reminder_status: activity?.reminder_status || (activity?.followup_completed ? "completed" : "scheduled")
  };
}

function localSalesmanAccount(db, lead) {
  const users = Array.isArray(db?.users) ? db.users : [];
  const assignedTo = String(lead?.assigned_to || "").trim();
  const assignedSalesman = String(lead?.assigned_salesman || "").trim().toLowerCase();
  const match = users.find(user =>
    (assignedTo && String(user.id || "") === assignedTo)
    || (assignedSalesman && [user.name, user.email, String(user.email || "").split("@")[0]].some(value => String(value || "").trim().toLowerCase() === assignedSalesman))
  );
  return match ? salesmanAccountSummary(match) : salesmanAccountSummary(null, {
    name: lead?.assigned_salesman || "",
    email: "",
    territory: lead?.territory || "",
    status: "active"
  });
...
async function supabaseSalesmanAccount(token, lead) {
  const assignedTo = String(lead?.assigned_to || "").trim();
  const assignedSalesman = String(lead?.assigned_salesman || "").trim();
  try {
    if (assignedTo) {
      const byId = await rest(`profiles?id=eq.${encodeURIComponent(assignedTo)}&select=id,full_name,email,role,territory,status,last_login_at&limit=1`, supabaseDataOptions(token));
      if (byId[0]) return salesmanAccountSummary(byId[0]);
    }
    if (assignedSalesman) {
      let rows = await rest(`profiles?role=eq.salesman&full_name=eq.${encodeURIComponent(assignedSalesman)}&select=id,full_name,email,role,territory,status,last_login_at&limit=1`, supabaseDataOptions(token)).catch(() => []);
      if (!rows[0]) {
        rows = await rest(`profiles?role=eq.salesman&email=eq.${encodeURIComponent(assignedSalesman)}&select=id,full_name,email,role,territory,status,last_login_at&limit=1`, supabaseDataOptions(token)).catch(() => []);
      }
      if (!rows[0]) {
        rows = await rest(`profiles?role=eq.salesman&full_name=ilike.*${encodeURIComponent(assignedSalesman)}*&select=id,full_name,email,role,territory,status,last_login_at&limit=1`, supabaseDataOptions(token)).catch(() => []);
      }
      if (rows[0]) return salesmanAccountSummary(rows[0]);
    }
  } catch {
    // Profile lookup should not block the AI summary.
  }
  return salesmanAccountSummary(null, {
    name: lead?.assigned_salesman || "",
    email: "",
    territory: lead?.territory || "",
    status: "active"
...
async function buildLeadSummaryBundle({ db, user, lead, supabaseEnabled }) {
  ...
  const followups = reminders.filter(activity =>
    activity.followup_completed
    || String(activity.reminder_status || "").toLowerCase() === "completed"
    || Boolean(activity.due_date)
  );
  const quotes = activities.filter(isQuoteActivity);
  const calls = activities.filter(isCallActivity);
  const emails = activities.filter(isEmailActivity);
  const meetings = activities.filter(isMeetingActivity);
  const stageChanges = activities.filter(isStageActivity);
  const noteEntries = activities.filter(isNoteActivity);
  const salesman = supabaseEnabled
...
function leadSummaryFingerprint(bundle) {
  return crypto
    .createHash("sha1")
    .update(JSON.stringify(buildLeadSummaryContext(bundle)))
    .digest("hex");
}
...
function assistantTableMissing(error, table) {
...
function assistantCompatibilityLogRow(user, entry, kind, leadId = "") {
  ...
  return {
    ...
    user_uid: user.id,
    user_role: user.role || "",
    scope: "ai_sales_assistant",
    company_id: /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(leadId || ""))
      ? leadId
      : null,
    action: `${kind}:${recordId}`,
    duration_ms: 0,
    status: "success",
    error: JSON.stringify(entry)
  };
}

function parseAssistantCompatibilityRows(rows, kind) {
  const prefix = `${kind}:`;
  return (Array.isArray(rows) ? rows : []).flatMap(row => {
    if (!String(row.action || "").startsWith(prefix)) return [];
    try {
      return [JSON.parse(String(row.error || "{}"))];
    } catch {
      return [];
    }
  });
...
async function saveAssistantCompatibilityEntry(user, entry, kind, leadId = "") {
  await rest("ai_action_log", {
    method: "POST",
    ...supabaseDataOptions(user.token),
    body: assistantCompatibilityLogRow(user, entry, kind, leadId)
  });
  return entry;
}

async function loadAssistantCompatibilityEntries(user, kind, id = "", limit = 100) {
  const action = id ? `${kind}:${id}` : `${kind}:%`;
  const operator = id ? "eq" : "like";
  const rows = await rest(
    `ai_action_log?user_uid=eq.${encodeURIComponent(user.id)}&scope=eq.ai_sales_assistant&action=${operator}.${encodeURIComponent(action)}&select=action,error,timestamp&order=timestamp.desc&limit=${limit}`,
    supabaseDataOptions(user.token)
  );
  return parseAssistantCompatibilityRows(rows, kind);
}
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/auth/refresh") {
    if (!supabaseEnabled) return sendJson(res, 401, { error: "Authentication required." });
    const payload = await readBody(req);
    const refreshToken = String(payload.refresh_token || "").trim();
    if (!refreshToken) return sendJson(res, 401, { error: "Authentication required." });
    try {
      ...
      if ([400, 401, 403].includes(Number(error?.status))) return sendJson(res, 401, { error: "Authentication required." });
      throw error;
    }
  }
  ...
  if (req.method === "GET" && url.pathname === "/api/ai-assistant/history") {
    if (supabaseEnabled) {
      try {
        ...
        return sendJson(res, 200, [...latest.values()].slice(0, 20)
  ...
  if (req.method === "POST" && url.pathname === "/api/ai/lead-summary") {
    const payload = await readBody(req);
    ...
    const startedAt = Date.now();
    const lead = supabaseEnabled
      ? await getSupabaseLead(user.token, leadId, user)
      : db.leads.find(item => item.id === leadId && leadBelongsToUser(item, user));
    if (!lead) return sendJson(res, 404, { error: "Company not found" });
    try {
      const bundle = await buildLeadSummaryBundle({ db, user, lead, supabaseEnabled });
      const fingerprint = leadSummaryFingerprint(bundle);
      const result = await runLeadAiSummary({
        bundle,
        openAiKey: OPENAI_API_KEY,
        model: OPENAI_LEAD_SUMMARY_MODEL
      });
      await recordAiActionLog(db, user, {
        leadId: lead.id,
        action: "lead_summary",
...
function serveStatic(req, res, url) {
  const publicFile = publicStaticFileForPath(url.pathname);
  if (!publicFile) {
    res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
...
server.publicStaticFileForPath = publicStaticFileForPath;
