...
- Sector assurance workflow: the enrichment review now shows `Suggested sector`, `Confidence`, `Reason`, and `Source` chips before users apply the sector. AI is expected to map only to the approved CRM sector list and should be treated as a source-backed recommendation, not a final decision.
